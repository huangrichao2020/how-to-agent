...
**不触发**（应路由到其他 skill）：
- 用户说"会议/开会" → 优先走 [lark-vc](../lark-vc/SKILL.md)
- 用户说"逐字稿/总结/待办/章节" → 走 [lark-vc +notes](../lark-vc/references/lark-vc-notes.md)
...
**示例 1 — 搜索某人的妙记：**
```bash
lark-cli minutes +search --owner "me" --keyword "周会"
```

**示例 2 — 提取 minute_token 并获取信息：**
```bash
# 从 URL https://xxx.feishu.cn/minutes/obcnABC123 提取 token
# 然后获取基础信息
lark-cli minutes minutes get --minute-token "obcnABC123"
```

**示例 3 — 下载妙记视频：**
```bash
lark-cli minutes +download --minute-token "obcnABC123" --dir ~/Downloads
```
...
| `authentication failed` / token 过期 | 认证失效 | 重新认证：参考 `lark-shared/SKILL.md` |
| 搜索返回空结果 | 关键词不匹配或无权限 | 尝试放宽关键词；确认用户有妙记访问权限 |
| `minute_token` 无效 | Token 错误或妙记已删除 | 用 `+search` 重新搜索获取有效 token |
| 下载超时 | 文件过大或网络不稳定 | 重试 3 次，每次间隔 5 秒；超过仍失败则提示用户 |
| 分页获取失败 | `page_token` 失效 | 从第一页重新获取 |
| 权限不足 | 缺少对应 scope | 检查权限表，确认应用已授权相应 scope |

**注意**：
- 下载链接有效期仅 1 天，过期后需重新生成
- `+search` 单次最多返回 200 条，结果总数无上限，需处理分页
- 妙记音视频文件可能较大，注意超时处理和重试机制
