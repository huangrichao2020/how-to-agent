---
name: 竞争格局分析
description: 用于构建竞争格局分析材料的框架，覆盖市场定位、竞争对手深挖、可比分析和战略归纳。适用于竞争格局研究、同行对比、市场地图绘制、战略复盘和投资备忘录支持。
---
...
## Exception handling & fallback patterns

**Data unavailability:**
- If a competitor is private with no public financials → use industry reports, news mentions of funding rounds, or sell-side estimates. Flag all as "[E]" estimates.
- If fiscal year alignment fails (e.g. Chinese companies use calendar year, US companies use non-calendar FY) → note the mismatch explicitly and normalize to trailing twelve months (TTM) where possible.
- If a key metric is genuinely unavailable for 2+ competitors → drop that metric from the comparative table rather than showing a table full of blanks. Add a note explaining why.

**Source conflicts:**
- When two sources disagree on a number → use the primary source (10-K > earnings call > sell-side > news). If both are primary sources, show the range ("$80-100B") and cite both.
- If market sizing estimates vary wildly (>3x difference between sources) → present the range, explain the methodology differences (top-down vs bottom-up), and note which is more conservative.

**Scope drift:**
- If the user's initial scope proves too broad (>15 competitors, >5 industries) → propose a phased approach: Tier 1 deep-dives first, Tier 2 summaries next. Get explicit approval before cutting scope.
- If the analysis reveals a previously unknown major competitor → flag it to the user with a brief note. Do NOT silently add it to the deck; ask first.

**Tool/environment failures:**
- If PowerPoint add-in is unavailable → fall back to generating a .pptx file via python-pptx. Notify the user of the mode switch.
- If slide rendering produces layout errors (overflow, overlap) → reduce content density: split one slide into two, or move detail to speaker notes rather than shrinking text below 14pt.

**Stale data warning:**
- If the most recent data point is >12 months old → add a "Data Currency" footnote on every slide containing that metric. Do not present stale numbers without the caveat.
