...
## 触发条件 / Trigger
当用户请求以下操作时触发此 Skill：
- 创建公司画像 / strip profile / company profile
- 制作投行 pitch book 幻灯片
- 生成公司介绍 PPT / 公司概览幻灯片
- 制作 quadrant layout 公司分析页
- 使用 PptxGenJS 创建专业幻灯片
## Workflow
...
### Data Unavailable
- **Private company with no public filings**: Use Crunchbase, company website, and news articles. Clearly mark estimates as "estimated" and note the source.
- **Financial data missing**: If Bloomberg/CapIQ unavailable, use Yahoo Finance or company IR page as fallback. Always cite the source.
- **Currency mismatch**: If data comes in mixed currencies, convert using current rates and note the exchange rate used.

### Rendering & Tool Failures
- **`soffice` not installed** (can't convert PPTX to image for review):
  ```bash
  # Install LibreOffice headless as fallback
  apt-get install -y libreoffice-impress poppler-utils
  # Alternative: use python-pptx to read slide XML directly for basic validation
  ```
- **PptxGenJS generation error**: Check Node.js version (requires 18+). Run `node --version`. If outdated: `nvm install 20`.
- **Font rendering issues**: If Arial is unavailable, fallback to system defaults. Test with `fc-list | grep -i arial`.

### Research Timeout & Fallback
- **Web search timeout (>30s)**: Retry once with 5s delay. If still failing, use cached/known data and flag as potentially outdated.
- **API rate limited (429)**: Wait with exponential backoff (2s → 4s → 8s). Max 3 retries. If all fail, use the best data already collected and inform the user.
- **Never proceed with placeholder data** — if real data cannot be obtained after retries, explicitly tell the user what's missing rather than generating fake numbers.

### Slide Quality Recovery
- **Text overlap detected during visual review**: Reduce font by 1pt → re-render → verify. Repeat until clean.
- **Chart data misaligned**: Verify data array lengths match label array lengths. Mismatched arrays cause silent chart corruption.
- **Color not rendering**: Use hex format `'#FF6B35'` not named colors. Some PptxGenJS versions don't support named colors.
