---
name: browser_cdp
description: "BYOB（Bring Your Own Browser）模式：连接到已运行的 Chrome 浏览器（含登录态）。当用户明确希望共享浏览器、复用登录态、或以暴露 CDP 端口的方式启动浏览器时使用本 skill。用户没有明确提到 CDP、共享浏览器或远程调试时，启动浏览器不得携带 cdp_port，也不得使用 connect_cdp。CDP 模式会暴露浏览器历史、Cookies 等敏感信息，使用前须告知用户；同一 workspace 同时只能运行或连接一个浏览器。"
categories:
  - automation
  - browser
metadata:
...
# BYOB 浏览器复用模式（Browser CDP 连接）

**BYOB（Bring Your Own Browser）** 让 AI agent 直接使用你已经在 Chrome 中打开的浏览器实例，复用所有登录态、cookie、session，而不是启动新的无头浏览器。

**核心价值**：解决了 agent 浏览器自动化的两大痛点：
1. **不需要重新登录** — 直接复用人类已建立的 session
2. **复用已有人类操作上下文** — agent 可以看到你正在看的页面

**Chrome DevTools Protocol（CDP）** 是实现 BYOB 的技术基础。本 skill 覆盖三类场景：
