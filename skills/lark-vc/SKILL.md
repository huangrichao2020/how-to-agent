...
| 错误现象 | 原因 | 解决方案 |
|----------|------|----------|
| `permission denied` / `scope not granted` | 缺少所需权限 scope | 对照权限表确认 scope；联系管理员在飞书开放平台授权 |
| `rate limit exceeded` | 超过 API 频率限制 | 等待后重试（建议间隔 2-5 秒）；批量操作时分批进行 |
| `meeting not found` | meeting_id 不存在或已过期 | 确认 ID 来源正确；会议可能被删除或超过保留期限 |
| `note not found` | 该会议未生成纪要 | 不是所有会议都有纪要；确认会议已结束且开启了录制/纪要功能 |

### 搜索结果为空

- 确认使用了至少一个筛选条件（`+search` 要求必填 filter）
- 确认查询的是**已结束**的会议，未开始的会议用 `lark-calendar` 技能
- 如果确定会议存在但搜不到，检查是否在正确的租户/应用上下文下
- 尝试缩小时间范围或使用更精确的关键词

### 纪要产物获取失败

- **`note_doc_token` 读取失败**：可能纪要文档已被删除或权限不足，尝试用 `lark-cli drive metas batch_query` 确认文档状态
- **`meeting_notes` 为空**：只有通过 `--calendar-event-ids` 路径才会返回用户绑定的会议纪要
- **逐字稿缺失**：不是所有会议都生成了逐字稿，需要确认会议开启了语音转写功能
- **妙记 (minutes) 找不到**：妙记来源于录制产物，未录制的会议没有妙记

### 超时与网络问题

- 飞书 API 默认超时 30 秒，大文档内容获取可能超时
- 如果 `docs +fetch` 超时，尝试 `--doc-format plain` 替代 `markdown` 减少返回体积
- 批量查询文档时每次最多 10 个，避免单次请求过大
- 遇到网络不稳定时，对关键操作（发送、删除）添加重试逻辑（最多 3 次，指数退避）

### 常见坑点

- **`+search` 和 calendar 的分工**：查询「开过的会」用 `vc +search`，查询「要开的会」用 `lark-calendar`。用户问「今天有哪些会议」时两个都要查
- **分页遗漏**：`+search` 返回结果可能分页，务必检查 `has_more` 字段并获取全部页面
- **文档 Token 类型混淆**：`note_doc_token`（AI 智能纪要）≠ `verbatim_doc_token`（逐字稿）≠ `meeting_notes`（用户绑定纪要），根据用户意图选择正确的 token
- **`schema` 优先原则**：调用任何原生 API 前必须先 `lark-cli schema vc.<resource>.<method>` 查看参数结构，不要猜测字段名
- **产物目录对齐**：手动下载的封面图等产物需放到 `./minutes/{minute_token}/` 目录下，与 `minutes +download` 保持一致

### 降级方案

- 如果 `lark-cli` 不可用，可通过飞书开放平台 REST API 直接调用（需要 tenant_access_token）
- 如果纪要文档无法读取，返回文档链接让用户手动打开查看
- 如果会议搜索失败，可退而使用 `lark-calendar` 搜索相关日程作为替代信息
