...
Community skills often trigger `CAUTION` verdicts due to `pip install` commands in `SKILL.md`. Use `force=True` and `skip_confirm=True` to proceed.
```python
...
The Skills Hub installs the **skill files** (SKILL.md, scripts) but **not** the Python dependencies. You must install them manually in the Hermes venv.
...
2. **Install Skill**: `do_install("official/mlops/whisper", ...)` (or write custom SKILL.md if official is missing).
