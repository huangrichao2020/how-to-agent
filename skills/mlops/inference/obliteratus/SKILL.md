...
| 错误类型 | 症状 | 解决方案 |
|---------|------|---------|
| OOM (显存不足) | `CUDA out of memory` | 添加 `--quantization 4bit` 或 `--large-model` 降低显存峰值 |
| 磁盘空间不足 | `No space left on device` | 清理 `/tmp` 缓存，确认 `--output-dir` 指向有足够空间的分区 |
| 超时失败 | 进程运行 >2h 无输出 | 检查 GPU 是否被其他进程占用；使用 `timeout` 命令设置超时兜底 |
| 模型加载失败 | `Error loading model weights` | 确认模型格式与架构兼容；尝试 `obliteratus info <model_name>` 验证支持状态 |
| 拒绝率未下降 | 输出仍有 refusal | 换用 `advanced` 方法替代 `basic`；检查 `obliteratus recommend` 获取更优参数 |
| 模型损坏 (ppl 飙升) | 困惑度 > 15% | 降低攻击性参数；用 `surgical` 替代 `aggressive` |

### 兜底策略

1. **fallback 到基础方法**: 当 `advanced` 方法失败时，fallback 到 `basic` 方法重试
2. **分段处理**: 大模型使用 `--large-model` 自动分段处理，避免单次显存溢出
3. **超时保护**: 建议用 `timeout 7200 obliteratus obliterate ...` 防止异常挂起
4. **结果验证**: 完成后必须运行 `obliteratus tourney <model>` 验证拒绝率是否真正下降
