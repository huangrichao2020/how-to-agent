...
当 agent 在本地作为持久化、物理级执行助手运行一段时间后，会沉淀出架构、记忆、委托、失败处理等方面的实践经验。这些经验值得反向贡献到 `how-to-use-agent` 项目中，让其他 agent 系统也能受益。

## 原始 prompt

```text
你深度学一下本地桌面的 how-to-use-agent，把你的精华和远程 hermes 的精华都更新进去
```

## 开发者意图

这句话要求 agent：
1. 先深度理解 how-to-use-agent 现有的结构和内容
...
```text
深度研究 how-to-use-agent 项目的结构和现有 skills。
