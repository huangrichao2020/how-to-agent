...
大多数 agent 平台优化的是**功能组合** —— 拖拽工作流、接入 LLM、交付一个聊天机器人。Dify、FastGPT、Coze、n8n 在这条路上做得非常出色。

`how-to-agent` 优化的是另一件事：**演化循环**。

真正的难题不是 prompt、不是模型、也不是工具。是这条闭环：
...
当这条循环跑起来，agent 会变得"更像 agent" —— 学会学习、学会遗忘、学会自我修正、学会重建，而不只是回答下一个 prompt。在项目自身的文化里，这条循环叫做**修真（xiuzhen）**。修真向 Agent OS 把学习、遗忘、自我修正和声誉当成一类**运行时一等公民**，而不是开发者的"之后再说"。

### 与功能组合平台相比

| 维度 | 功能组合平台 | 修真向 Agent OS |
|---|---|---|
| 记忆 | 向量库 + 检索 | 分层记忆 + 触发式注入 + 证据链 + 漂移修正 |
| 技能 | 插件市场 | 触发式技能，以"是否降低上下文成本 / 提升稳定性"为入选标准 |
| 自我修改 | 手动改配置 | 可见讨论 + 版本化手册 + 回滚路径 + 相邻系统冻结 |
| 演化 | 运营驱动的发布 | 原生循环：信号 → 实践 → 方法 → 技能 → 本能 → 下一轮信号 |

右边那一列不是路线图。每一行对应的能力都已经在本仓库描述的 agent 项目里以某种形式跑在生产中。

### 五件套生态

`how-to-agent` 是手册，不是整个 OS。OS 由五个仓库拼成，每个仓库都用"喂给演化循环"的方式挣到自己的位置：

- **[GenericAgent](https://github.com/huangrichao2020/GenericAgent)** —— 运行时
- **[hermes-agent](https://github.com/huangrichao2020/hermes-agent)** —— 修真循环的活体样本
- **how-to-agent**（本仓库）—— 方法论手册
- **[agent-systems-patterns](https://github.com/huangrichao2020/agent-systems-patterns)** —— 来自真实系统的模式雷达
- **[uwillberich](https://github.com/huangrichao2020/uwillberich)** —— 修真回路在金融域的产品化

只读一个仓库就读这本。想看循环在跑，去读另外四个。
...
### 2. 前置链路分流和门控优先于 prompt 补丁

不要用又一段写死在运行时代码里的 prompt，去解决路由、权限、通道选择、群聊/私聊边界、投递目标、工具契约或安全门控问题。前置链路应该先识别场景、选择通道、完成能力门控，并产出结构化的任务/投递契约，模型只负责在契约内组织语言。

提示词类内容应该抽成独立、可热加载的 Markdown 模板。运行时代码只加载命名模板，不把行为补丁藏在 Python、TypeScript 或 shell 字符串里。凡是需要稳定性的行为，先问：能不能由总线、路由器、policy 对象、状态机、typed target 或工具契约保证？prompt 只负责语气、判断和最终表达。
